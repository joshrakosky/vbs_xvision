...
export default function BagsPage() {
  ...
  const categories = ROUTE_CATEGORIES.bags
  ...
  useEffect(() => {
    ...
    loadProducts()
    const existing = loadCart().find((i) => i.category === 'bags')
  ...
  const handleContinue = () => {
    if (!selectedProduct) {
      setError('Please select one backpack')
      return
    }
    ...
    saveCart(updated)
    setError('')
    const next = getNextRoute('bags')
    if (next) router.push(`/${next}`)
  }

  const handleBack = () => {
    const prev = getPrevRoute('bags')
        ...
        <div className="max-w-2xl mx-auto">
          <div className="bg-white rounded-lg shadow-lg p-8">
            ...
            <div className="space-y-4 mb-6">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Backpack</label>
                <select
                ...
                >
                  <option value="">-- Choose a backpack --</option>
                  {products.map((p) => (
