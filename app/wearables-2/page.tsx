...
export default function Wearables2Page() {
  ...
  const categories = ROUTE_CATEGORIES['wearables-2']
  ...
  const handleContinue = () => {
    ...
    const cart = loadCart()
    const nonWearables = cart.filter(
      (i) => !i.scrubTopId && !(i.productId && WEARABLE_CATEGORIES.includes(i.category || ''))
    )
    const existingWearables = cart.filter(
      (i) => i.scrubTopId || (i.productId && WEARABLE_CATEGORIES.includes(i.category || ''))
    )
    const updated = [...nonWearables, ...existingWearables, item]
    saveCart(updated)
    setError('')
    const next = getNextRoute('wearables-2')
    if (next) router.push(`/${next}`)
  }

  const handleBack = () => {
    const prev = getPrevRoute('wearables-2')
